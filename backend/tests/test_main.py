...
class TestAddMood:
    def test_add_mood_with_all_fields(self) -> None:
    ...
    def test_add_mood_without_mood_defaults_to_unknown(self) -> None:
        response = client.post("/add", json={})

        assert response.status_code == 200
        assert response.json()["entry"]["mood"] == "unknown"

    def test_add_mood_without_note_defaults_to_empty_string(self) -> None:
        response = client.post("/add", json={"mood": "happy"})

        assert response.status_code == 200
        assert response.json()["entry"]["note"] == ""

    def test_add_mood_with_invalid_type_returns_422(self) -> None:
        response = client.post("/add", json={"mood": 123})

        assert response.status_code == 422

    def test_add_mood_exceeding_max_length_returns_422(self) -> None:
        response = client.post("/add", json={"mood": "x" * 101})

        assert response.status_code == 422

    def test_add_mood_includes_required_fields(self) -> None:
...
class TestListMoods:
    ...
    def test_list_moods_returns_all_added_entries(self) -> None:
        client.post("/add", json={"mood": "happy"})
        client.post("/add", json={"mood": "sad"})
    ...
    def test_list_moods_includes_age_in_seconds(self) -> None:
    ...
    def test_list_moods_sorted_by_timestamp_descending(self) -> None:
        import time
