...
Estáis en la rama `ejercicio-1`, que parte de `main` con los ficheros de contexto ya creados para la raíz y el backend FastAPI.
...
- `backend/AGENTS.md` — contexto específico del backend FastAPI: endpoints, arquitectura, comandos.
- `backend/CLAUDE.md` — puntero a `@AGENTS.md` en el scope de backend.
- `backend/docs/reglas.md` — reglas ejecutables del backend (tests con pytest, linter ruff, tipos con mypy).
...
Añadir una regla ejecutable al backend que el agente pueda verificar con un test. Por ejemplo: "todos los endpoints nuevos requieren al menos un test de integración con `pytest` y el cliente de test de FastAPI".

**Hecho cuando:**
- [ ] La regla está en `backend/docs/reglas.md` con un criterio verificable que el agente pueda comprobar ejecutando `uv run pytest`.
