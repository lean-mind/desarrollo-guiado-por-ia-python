...
- `backend/pyproject.toml` — configuración de ruff (linter) y mypy (tipos) para el backend FastAPI.
...
Usar el ciclo `/plan → /execute` para añadir un campo `created_at` a los moods: se asigna automáticamente al crearlo, se devuelve en las respuestas y hay tests que lo cubren.
...
- [ ] La feature `created_at` funciona end-to-end con tests verdes y `pytest` pasa sin errores.

## Extra (si acabáis antes)

Añadir una regla nueva al linter o al hook (p.ej., que falle si hay un `print()` en el código de producción) y volver a ejecutar el ciclo para ver cómo cambia la dinámica del agente.

**Hecho cuando:**
- [ ] La nueva regla está configurada en `pyproject.toml` o en `.ai/hooks/pre-commit`.
...
```bash
git diff ejercicio-4..solucion-4 -- backend/
```
